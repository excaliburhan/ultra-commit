  ...
  return ProgressHandler.withProgress('', async (progress) => {
    try {
      ...
      try {
        ...
        if (aiProvider === 'bailian') {
          const bailianApiKey = configManager.getConfig<string>(ConfigKeys.BAILIAN_API_KEY);
          if (!bailianApiKey) {
        ...
        } else if (aiProvider === 'bailian') {
          errorMessage = `Bailian API error: ${err.message}`;
        }
